...
description: "Regular Expressions - REGEX - are a way of describing patterns of text."
lead: "A 'regular expression' is a description of a pattern; if we can describe patterns of data, we can do some powerful manipulations."
...
which would find all instances of "dogs and cats" in your document, your program would remember "dogs" is group 1, "and" is group 2, and "cats" is group 3. Sublime Text remembers them as `\1`, `\2`, and `\3` for each group respectively (Atom would use `$1`, `$2`, `$3`).
...
The vocabulary of regular expressions is pretty large, but there are many cheat sheets for regex online. Regex patterns can also be used programmatically, inside languages like R or Python, or in combination with commands typed at the command prompt, but you **will* need to research the idiosyncracies for each implementation.
...
and the entire line is placed within a parenthetical group. Since this the first group in our search expression, we can replace that group with `\1` (assuming you're using Sublime Text) and put the tilde in front of it like so: `~\1`.
...
**Read the next bit through before you do anything.** You will start by removing the page number after the year and the comma between the year and the month-date. To do this, we could look for the year on each line by using the regex:
...
But it's still pretty messy - it was, after all, OCR'd in the first place. We'll fix that in the [open refine](docs/tutorials/open-refine/) exercise.
